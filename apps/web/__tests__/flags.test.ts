...
describe('AAA_POLISH_DEFAULT flag', () => {
  ...
  it('is true when env var is "true"', async () => {
    ...
    vi.resetModules()
    const { AAA_POLISH_DEFAULT } = await import('@/lib/flags')
    expect(AAA_POLISH_DEFAULT).toBe(true)
  })
  ...
  it('is false when env var is "false"', async () => {
    ...
    vi.resetModules()
    const { AAA_POLISH_DEFAULT } = await import('@/lib/flags')
    expect(AAA_POLISH_DEFAULT).toBe(false)
  })
